...
Command.prototype.request = function (method, context, href, params, callback) {
    ...
    if (Array.isArray(opts.proxy) || util.isFunction(opts.proxy)) {
        proxies = opts.proxies = opts.proxy;
    }

    if (Array.isArray(opts.proxies)) {
        if (proxies.index === undefined || ++proxies.index >= proxies.length) {
            proxies.index = 0;
        }

        opts.proxy = proxies[proxies.index];
    } else if (util.isFunction(opts.proxies)) {
        // acquire a proxy through error-first callback
...
Command.register = contextCommand;
